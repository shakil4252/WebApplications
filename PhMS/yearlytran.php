  ...
  <div class="content-wrapper">
          ...
          <div class="box box-primary">
			...
			<?php
				...
				$sql=$conn->query("SELECT sum(sale_payment) FROM sales WHERE sale_date like '______$y' ;");
				$xsql=$conn->query("SELECT sum(expense_payment) FROM expenses WHERE expense_date like '______$y' ;");
				if($xsql->rowCount()>0){
					$xrow = $xsql->fetch(PDO::FETCH_ASSOC);
					$date="20".$y;
					$expense=$xrow["sum(expense_payment)"];
					$row = $sql->fetch(PDO::FETCH_ASSOC);
					$sale=$row["sum(sale_payment)"];
					$profit=$sale-$expense;
			?>	
			<hr>
				...
				<?php }?>
				<?php
				$sql2=$conn->query("select sum(sale_price),sum(sale_payment),sale_date 
				from sales where sale_date like '______$y' group by sale_date");
          ...
          <hr>
					<table class="table table-bordered rowBorder">
						 <h3 class="profile-username text-center">Monthly Sale</h3>
						<tr>
					...
					while($row2 = $sql2->fetch(PDO::FETCH_ASSOC)) {
						...
						<tr>
							<td>
								<?php echo $row2["sale_date"] ?></td>
							<td>
								<?php echo $row2["sum(sale_price)"] ?></td>
							<td	>
								<?php echo $row2["sum(sale_payment)"] ?></td>
						</tr>
					<?php } ?>
					</table>
						<?php } ?>
				<?php
				$sql3=$conn->query("SELECT sum(expense_amount),sum(expense_payment),expense_date
				FROM `expenses` WHERE expense_date like '______$y' group by expense_date");
          ...
          <hr>
					<table class="table table-bordered rowBorder">
						 <h3 class="profile-username text-center">Monthly Expenses</h3>
						<tr>
							<td class="bold">Date</td>
					...
					while($row3 = $sql3->fetch(PDO::FETCH_ASSOC)) {
						...
						<tr>
							<td>
								<?php echo $row3["expense_date"] ?></td>
							<td>
								<?php echo $row3["sum(expense_amount)"] ?></td>
							<td	>
								<?php echo $row3["sum(expense_payment)"] ?></td>
						</tr>
					<?php } ?>
					</table>
						<?php } ?>
