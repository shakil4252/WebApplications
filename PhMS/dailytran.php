  ...
  <div class="content-wrapper">
          ...
          <div class="box box-primary">
			...
			<?php
				...
				$sql=$conn->query("SELECT expense_date,sum(expense_payment) FROM expenses
                where expense_date='$d-$m-$y' ");
				$sql2=$conn->query("SELECT `sale_date`,sum(sale_payment) FROM sales
                where `sale_date`='$d-$m-$y' ");
				if($sql->rowCount()>0 && $sql2->rowCount()>0){
					$row = $sql->fetch(PDO::FETCH_ASSOC);
					$row2 = $sql2->fetch(PDO::FETCH_ASSOC);
					$date=$mydate;
					$expense=$row["sum(expense_payment)"];
					$sale=$row2["sum(sale_payment)"];
					$profit=$sale-$expense;
			?>	
			<hr>
				<table class="table table-bordered rowBorder">
						 <h3 class="profile-username text-center">Daily Transiction</h3>
					<tr>
						<td>Date</td>
						<td><?php echo $date?></td>
					</tr>
				...
				<?php }?>
				<?php
				$sql3=$conn->query("SELECT sale_no,customer_id,product_name,sale_price,sale_payment
				FROM `sales` where sale_date='$d-$m-$y' order by sale_no");
          ...
          <hr>
					<table class="table table-bordered rowBorder">
						 <h3 class="profile-username text-center">Daily Sale</h3>
						<tr>
							<td class="bold">Sale NO</td>
					...
					while($row3 = $sql3->fetch(PDO::FETCH_ASSOC)) {
						...
						<tr>
							<td>
								<?php echo $row3["sale_no"] ?>
							</td>
							<td>
								<?php echo $row3["customer_id"] ?></td>
							<td>
								<?php echo $row3["product_name"] ?></td>
							<td>
								<?php echo $row3["sale_price"] ?></td>
							<td	>
								<?php echo $row3["sale_payment"] ?></td>
						</tr>
					<?php } ?>
					</table>
						<?php } ?>
				<?php
				$sql4=$conn->query("SELECT expense_no,supplier_id,product_name,expense_amount,expense_payment
				FROM `expenses` where expense_date='$d-$m-$y' order by expense_no");
          ...
          <hr>
					<table class="table table-bordered rowBorder">
						 <h3 class="profile-username text-center">Daily Expenses</h3>
						<tr>
							<td class="bold">Expense NO</td>
					...
					while($row4 = $sql4->fetch(PDO::FETCH_ASSOC)) {
						...
						<tr>
							<td>
								<?php echo $row4["expense_no"] ?>
							</td>
							<td>
								<?php echo $row4["supplier_id"] ?></td>
							<td>
								<?php echo $row4["product_name"] ?></td>
							<td>
								<?php echo $row4["expense_amount"] ?></td>
							<td	>
								<?php echo $row4["expense_payment"] ?></td>
						</tr>
					<?php } ?>
					</table>
						<?php } ?>
